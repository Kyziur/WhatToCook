...
### Repository Workflow: WhatToCook

Use two passes in this repository:

- `inner-loop`: while implementing, run the smallest relevant scope first.
- `handoff`: run the full required sequence once, immediately before final handoff of the complete user-requested task, unless the user explicitly agrees to a blocker or intentionally narrower scope. A delivery package is not an intermediate edit or a small fix within the same task.

Handoff sequence:

1. `dotnet build WhatToCook.slnx`
2. `dotnet test WhatToCook.slnx` or the intentionally scoped filter for the package
...
- Run `dotnet build` and `dotnet test` sequentially, never in parallel.
- For planner-only, shopping-only, import-only, or similar focused changes, use `agent-browser` as the default inner-loop check. Run a scoped Playwright subset only to diagnose an E2E-specific failure, when E2E tests or browser flow changed, or when the user explicitly requests it:
  `PLAYWRIGHT_HTML_OPEN=never npx playwright test --config=playwright.config.ts --grep "<scenario regex>" --workers=1 --reporter=line`
- If a scoped run fails, fix it and rerun that same scope before expanding to broader validation.
